...
public class PlayService extends Service implements MediaPlayer.OnCompletionListener {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(mode != null){
                if(mode.equals("start")){
                    try{
                        //재생 중이라면 (재생중인지 물어보려면 플레이어가 널이 아니고, 플레이어가 플레이중인지 이런방식으로 물어봐야함)
                        if(player != null && player.isPlaying()){
                            //재생을 중지하고 메모리 정리
                            player.stop();
                            //메모리 해제
                            //안드로이드 시스템은 C언어로 만들어져있기 때문에 메모리 해제를 따로 해주어야합니다.
                            player.release();
                        ...
                        player.start();
                        //리시버를 다시 호출
                        Intent aIntent = new Intent("naver.rlgns1129.PLAY_TO_ACTIVITY");
                        //음원이 재생 중인지 확인해 줄 수 있도록 해주기 위한 값
                        aIntent.putExtra("mode", "start");
                        //재생중인 위치를 알 수 있도록 해주기 위한 값
    ...
    //MediaPlayer.OnCompletionListener 인터페이스의 재생이 종료되었을 때 호출되는 메소드
    @Override
    public void onCompletion(MediaPlayer mediaPlayer) {
        //종료 되었으므로 종료 되었다고 방송을 하고 서비스를 중지
        Intent intent = new Intent("naver.rlgns1129.PLAY_TO_ACTIVITY");
        intent.putExtra("mode", "stop");
        sendBroadcast(intent);
        //서비스를 중지 - 이 메소드를 호출하지 않으면 서비스는 계속 살아있음 ( 저런 상황을 메모리 누수라고 한다.)
        stopSelf();
    }
    ...
    public void onCreate(){
        super.onCreate();
        //리시버 등록
        registerReceiver(receiver, new IntentFilter("naver.rlgns1129.PLAY_TO_SERVICE"));
    }
